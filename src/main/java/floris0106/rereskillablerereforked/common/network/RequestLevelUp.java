    ...
    public void handle(Supplier<NetworkEvent.Context> context)
    {
        context.get().enqueueWork(() ->
        {
            ...
            if (skillModel.getSkillLevel(skill) < Config.getMaxLevel())
            {
                ...
                {
                    int fragments = ContainerHelper.clearOrCountMatchingItems((Container) player.inventoryMenu, stack -> stack.getItem() == Items.SKILL_FRAGMENT.get(), 0, true);
                    if (player.isCreative() || fragments >= cost)
                    {
                        if (!player.isCreative())
                        	ContainerHelper.clearOrCountMatchingItems((Container) player.inventoryMenu, stack -> stack.getItem() == Items.SKILL_FRAGMENT.get(), cost, false);
